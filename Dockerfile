...
# Health check to ensure container is running (respects WEBHOOK_PORT)
HEALTHCHECK --interval=30s --timeout=10s --start-period=90s --retries=5 \
    CMD python -c 'import os,urllib.request; p=os.environ.get("WEBHOOK_PORT","9898"); urllib.request.urlopen("http://127.0.0.1:"+p+"/", timeout=5).read()' || exit 1
